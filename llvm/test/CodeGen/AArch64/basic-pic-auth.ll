; RUN: llc -mtriple=aarch64-linux-gnu -global-isel=0 -verify-machineinstrs -relocation-model=pic -mattr=+pauth %s -o - | FileCheck %s
; RUN: llc -mtriple=aarch64-linux-gnu -global-isel=1 -verify-machineinstrs -relocation-model=pic -mattr=+pauth %s -o - | FileCheck %s
