; RUN: llc -mtriple=aarch64-none-linux-gnu -global-isel=0 -relocation-model=pic -mattr=+pauth -o - %s | FileCheck %s
; RUN: llc -mtriple=aarch64-none-linux-gnu -global-isel=1 -relocation-model=pic -mattr=+pauth -o - %s | FileCheck %s
; RUN: llc -mtriple=aarch64-none-linux-gnu -global-isel=0 -relocation-model=pic -filetype=obj -mattr=+pauth -o /dev/null %s
