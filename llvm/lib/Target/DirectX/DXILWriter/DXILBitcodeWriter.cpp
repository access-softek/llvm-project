...
public:
  ...
  DXILBitcodeWriter(const Module &M, SmallVectorImpl<char> &Buffer,
      ...
      : I8Ty(Type::getInt8Ty(M.getContext())),
        ...
        PointerMap(PointerTypeAnalysis::run(M)), DebugInfo(DI) {
...
void BitcodeWriter::writeModule(const Module &M,
  ...
  DXILBitcodeWriter ModuleWriter(M, Buffer, StrtabBuilder, *Stream, DI);
  ModuleWriter.write();
}
...
void DXILBitcodeWriter::writeDICompileUnit(const DICompileUnit *N,
                                           ...
                                           unsigned Abbrev) {
  Record.push_back(N->isDistinct());
  Record.push_back(N->getSourceLanguage().getName());
  Record.push_back(VE.getMetadataOrNullID(N->getFile()));
  ...
  Record.push_back(VE.getMetadataOrNullID(DebugInfo.CUSubprograms.lookup(N)));
...
void DXILBitcodeWriter::writeFunctionMetadataAttachment(const Function &F) {
  ...
  if (!MDs.empty()) {
    for (const auto &I : MDs) {
      if (I.first == LLVMContext::MD_dbg) continue;
