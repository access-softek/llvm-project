...
static bool matchFormAuthLoad(MachineInstr &MI, MachineRegisterInfo &MRI,
  ...
  if (LoadPtrDef->getOpcode() == TargetOpcode::G_INTTOPTR) {
    ...
    if (!mi_match(IntPtr, MRI, m_any_of(
          m_GAdd(m_Reg(BasePtr), m_ICst(MatchInfo.Offset)),
          m_Reg(BasePtr))))
      return false;

  } else if (!mi_match(*LoadPtrDef, MRI, m_GPtrAdd(m_GIntToPtr(m_Reg(BasePtr)),
                                                   m_ICst(MatchInfo.Offset)))) {
      return false;
  }

  MachineInstr *AUT = getOpcodeDef(TargetOpcode::G_INTRINSIC, BasePtr, MRI);
  if (!AUT || AUT->getIntrinsicID() != Intrinsic::ptrauth_auth)
    return false;
