...
private:
  ...
  // by a constant value
  bool rewriteAddressCalculation(LdStInfo &First, LdStInfo &Second,
                                 int32_t RegStride) const;
...
ARMPostIndexingOpt::getDataTypeAndBaseIndex(Instruction *I) const {
...
int32_t ARMPostIndexingOpt::guessCustomAccessStride(
    ArrayRef<LdStInfo> Instructions) const {
...
bool ARMPostIndexingOpt::rewriteAddressCalculation(LdStInfo &First,
                                                   LdStInfo &Second,
                                                   int32_t RegStride) const {
  ...
  if (FirstBaseTy == SecondBaseTy && Stride % FirstElementSize == 0) {
    ...
    Value *FirstBaseBytePtr =
        IRB.CreateBitCast(FirstBase, BytePtrTy, "oldbase.byteptr");
    Value *NewSecondBaseBytePtr =
        IRB.CreateConstGEP1_32(FirstBaseBytePtr, Stride, "postinc.byteptr");
    NewSecondBase =
