...
public:
  ...
  template <int SignificantBits, int ShiftAmount>
  bool SelectAddrModeImm(SDValue N, SDValue &Base, SDValue &OffImm);
  ...
  bool SelectAddrModeVLDSTIndexedPseudo(SDNode *Parent, SDValue N,
                                        SDValue &Addr, SDValue &Offset,
                                        SDValue &Align);
...
private:
  ...
  SDValue CreateBasePlusOffset(SDValue Base, int64_t Offset, SDValue Cond,
                               SDValue CondReg);
  SDValue ReplacePseudoVLDST(MachineSDNode *OldNode, bool IsPostIndexed,
                             SDValue Base, SDValue Cond, SDValue CondReg,
                             SDValue ChainIn);
  void RewriteVLDSTPseudoInstructions(SDValue Base);
...
static unsigned getVLDSTWithoutOffset(unsigned Opcode) {
  switch (Opcode) {
  default: return 0;
  case ARM::VLD1d8_indexed:  return ARM::VLD1d8;
  case ARM::VLD1d16_indexed: return ARM::VLD1d16;
  case ARM::VLD1d32_indexed: return ARM::VLD1d32;
  case ARM::VLD1d64_indexed: return ARM::VLD1d64;
  case ARM::VLD1q8_indexed:  return ARM::VLD1q8;
  case ARM::VLD1q16_indexed: return ARM::VLD1q16;
  case ARM::VLD1q32_indexed: return ARM::VLD1q32;
  case ARM::VLD1q64_indexed: return ARM::VLD1q64;
  case ARM::VST1d8_indexed:  return ARM::VST1d8;
  case ARM::VST1d16_indexed: return ARM::VST1d16;
  case ARM::VST1d32_indexed: return ARM::VST1d32;
  case ARM::VST1d64_indexed: return ARM::VST1d64;
  case ARM::VST1q8_indexed:  return ARM::VST1q8;
  case ARM::VST1q16_indexed: return ARM::VST1q16;
  case ARM::VST1q32_indexed: return ARM::VST1q32;
  case ARM::VST1q64_indexed: return ARM::VST1q64;
  }
}

static unsigned getVLDSTImplicitlyPostIndexed(unsigned Opcode) {
  switch (Opcode) {
  default: return 0;
  case ARM::VLD1d8_indexed:  return ARM::VLD1d8wb_fixed;
  case ARM::VLD1d16_indexed: return ARM::VLD1d16wb_fixed;
  case ARM::VLD1d32_indexed: return ARM::VLD1d32wb_fixed;
  case ARM::VLD1d64_indexed: return ARM::VLD1d64wb_fixed;
  case ARM::VLD1q8_indexed:  return ARM::VLD1q8wb_fixed;
  case ARM::VLD1q16_indexed: return ARM::VLD1q16wb_fixed;
  case ARM::VLD1q32_indexed: return ARM::VLD1q32wb_fixed;
  case ARM::VLD1q64_indexed: return ARM::VLD1q64wb_fixed;
  case ARM::VST1d8_indexed:  return ARM::VST1d8wb_fixed;
  case ARM::VST1d16_indexed: return ARM::VST1d16wb_fixed;
  case ARM::VST1d32_indexed: return ARM::VST1d32wb_fixed;
  case ARM::VST1d64_indexed: return ARM::VST1d64wb_fixed;
  case ARM::VST1q8_indexed:  return ARM::VST1q8wb_fixed;
  case ARM::VST1q16_indexed: return ARM::VST1q16wb_fixed;
  case ARM::VST1q32_indexed: return ARM::VST1q32wb_fixed;
  case ARM::VST1q64_indexed: return ARM::VST1q64wb_fixed;
  }
}

static bool isVLDSTIndexedPseudo(unsigned Opcode) {
  assert((getVLDSTImplicitlyPostIndexed(Opcode) != 0) ==
             (getVLDSTWithoutOffset(Opcode) != 0) &&
         "Functions are out of sync");
  return getVLDSTWithoutOffset(Opcode) != 0;
}
...
template <int SignificantBits, int ShiftAmount>
bool ARMDAGToDAGISel::SelectAddrModeImm(SDValue N, SDValue &Base,
                                        SDValue &OffImm) {
  ...
  if (ConstantSDNode *RHS = dyn_cast<ConstantSDNode>(N.getOperand(1))) {
    ...
    if (isShiftedInt<SignificantBits, ShiftAmount>(std::abs(RHSC))) {
      Base   = N.getOperand(0);
...
bool ARMDAGToDAGISel::SelectAddrModeVLDSTIndexedPseudo(SDNode *Parent,
                                                       SDValue N, SDValue &Addr,
                                                       SDValue &Offset,
                                                       SDValue &Align) {
  // TODO: Maybe handle other ADDri/SUBri immediates as offsets
  if (!SelectAddrModeImm<8, 2>(N, Addr, Offset))
    return false;

  MemSDNode *MemN = cast<MemSDNode>(Parent);
  const unsigned DefaultAlign =
      CurDAG->getEVTAlign(MemN->getMemoryVT()).value();
  unsigned Alignment = MemN->getAlignment();
  if (Alignment >= 16)
    Alignment = 16;
  else if (Alignment >= 8 && Alignment > DefaultAlign)
    Alignment = 8;
  else
    Alignment = 0;
  Align = CurDAG->getTargetConstant(Alignment, SDLoc(N), MVT::i32);
...
void ARMDAGToDAGISel::PostprocessISelDAG() {
  SmallSet<SDValue, 4> VLDSTBaseAddresses;
  for (SDNode &Node : CurDAG->allnodes()) {
    if (Node.isMachineOpcode() &&
        isVLDSTIndexedPseudo(Node.getMachineOpcode())) {
      VLDSTBaseAddresses.insert(Node.getOperand(0));
    }
  }
  for (SDValue Base : VLDSTBaseAddresses)
    RewriteVLDSTPseudoInstructions(Base);
}
...
SDValue ARMDAGToDAGISel::CreateBasePlusOffset(SDValue Base, int64_t Offset,
                                              SDValue Cond, SDValue CondReg) {
  SDLoc dl(Base);
  const bool IsThumb = Subtarget->isThumb();
  if (isa<ConstantPoolSDNode>(Base)) {
    SDValue Ops[] = {Base, Cond, CondReg};
    const unsigned LEAOpcode = IsThumb ? ARM::t2LEApcrel : ARM::LEApcrel;
    SDNode *LEANode = CurDAG->getMachineNode(LEAOpcode, dl, MVT::i32, Ops);
    EnforceNodeIdInvariant(LEANode);
    Base = SDValue(LEANode, 0);
  }

  if (Offset == 0 && !isa<FrameIndexSDNode>(Base))
    return Base;

  unsigned Opcode = Offset >= 0 ? (IsThumb ? ARM::t2ADDri : ARM::ADDri)
                                : (IsThumb ? ARM::t2SUBri : ARM::SUBri);
  SDValue OffsetValue =
      CurDAG->getTargetConstant(std::abs(Offset), dl, MVT::i32);
  SDValue Ops[] = {Base, OffsetValue, Cond, CondReg,
                   CurDAG->getRegister(0, MVT::i32)};
  SDNode *AddLikeNode = CurDAG->getMachineNode(Opcode, dl, MVT::i32, Ops);
  EnforceNodeIdInvariant(AddLikeNode);
  return SDValue(AddLikeNode, 0);
}

static bool isLoadNode(const ARMSubtarget *Subtarget, const SDNode *Node) {
  const unsigned Opcode = Node->getMachineOpcode();
  return Subtarget->getInstrInfo()->get(Opcode).mayLoad();
}

SDValue ARMDAGToDAGISel::ReplacePseudoVLDST(MachineSDNode *OldNode,
                                            bool IsPostIndexed, SDValue Base,
                                            SDValue Cond, SDValue CondReg,
                                            SDValue ChainIn) {
  SDLoc dl(OldNode);
  const unsigned Opcode = OldNode->getMachineOpcode();

  const bool IsLoad = isLoadNode(Subtarget, OldNode);
  const EVT VecType =
      IsLoad ? OldNode->getValueType(0) : OldNode->getOperand(3).getValueType();
  const unsigned NewOpcode = IsPostIndexed
                                 ? getVLDSTImplicitlyPostIndexed(Opcode)
                                 : getVLDSTWithoutOffset(Opcode);

  SmallVector<EVT, 3> ValueTypes;
  if (IsLoad)
    ValueTypes.push_back(VecType);
  if (IsPostIndexed)
    ValueTypes.push_back(MVT::i32);
  ValueTypes.push_back(MVT::Other);

  SmallVector<SDValue, 6> Ops;
  Ops.append({Base, OldNode->getOperand(2)});
  if (!IsLoad)
    Ops.push_back(OldNode->getOperand(3));
  Ops.append({Cond, CondReg, ChainIn});

  MachineSDNode *NewNode =
      CurDAG->getMachineNode(NewOpcode, dl, ValueTypes, Ops);
  SDValue OldChainOut(OldNode, IsLoad ? 1 : 0);
  SDValue NewChainOut(NewNode, ValueTypes.size() - 1);
  CurDAG->setNodeMemRefs(NewNode, OldNode->memoperands());

  ReplaceUses(OldChainOut, NewChainOut);
  if (IsLoad)
    ReplaceUses(SDValue(OldNode, 0), SDValue(NewNode, 0));

  CurDAG->RemoveDeadNode(OldNode);

  return SDValue(NewNode, IsLoad ? 1 : 0);
}

static SDNode *getTheOnlyChainOutUser(SDValue ChainOut, bool &HasAnyUse) {
  SDNode *Result = nullptr;
  SDNode *N = ChainOut.getNode();
  HasAnyUse = false;
  for (auto I = N->use_begin(), End = N->use_end(); I != End; ++I) {
    if (I.getUse().getResNo() != ChainOut.getResNo())
      continue; // Use of some other value of N
    HasAnyUse = true;
    if (Result != nullptr)
      return nullptr; // Multiple uses found
    Result = *I;
  }
  // At this point, at most one use was found
  return Result;
}

void ARMDAGToDAGISel::RewriteVLDSTPseudoInstructions(SDValue Base) {
  SmallVector<MachineSDNode *, 16> Pseudos;
  SDNode *BaseNode = Base.getNode();
  for (auto I = BaseNode->use_begin(), E = BaseNode->use_end(); I != E; ++I) {
    // Skip irrelevant nodes
    if (!I->isMachineOpcode() || !isVLDSTIndexedPseudo(I->getMachineOpcode()))
      continue;
    // Skip uses of chain output of `Base.getNode()`, etc.
    // If `Base` is used multiple times, as operand 0 *and some other* operand
    // (f.e., chain input) add it only once.
    if (I.getUse().get() != Base)
      continue;
    Pseudos.push_back(cast<MachineSDNode>(*I));
  }

  LLVM_DEBUG(dbgs() << "ARM ISEL: Rewriting " << Pseudos.size()
                    << " pseudo instructions with base ");
  LLVM_DEBUG(Base->dump(CurDAG));

  // Sort in increasing offset order
  sort(Pseudos, [](SDNode *A, SDNode *B) {
    return A->getConstantOperandAPInt(1).getSExtValue() <
           B->getConstantOperandAPInt(1).getSExtValue();
  });

  // Iteratively take the first instruction as a reference, then collect all
  // other instructions that can be chained with it.
  while (!Pseudos.empty()) {
    SmallVector<MachineSDNode *, 16> Worklist;
    MachineSDNode *Reference = Pseudos.front();
    const unsigned NumOperands = Reference->getNumOperands();
    SDValue Cond = Reference->getOperand(NumOperands - 3);
    SDValue CondReg = Reference->getOperand(NumOperands - 2);
    SDValue ChainIn = Reference->getOperand(NumOperands - 1);
    SDValue ChainOut(Reference, isLoadNode(Subtarget, Reference) ? 1 : 0);

    bool HasRefChainOutUses;
    SDNode *TheOnlyRefChainOutUser =
        getTheOnlyChainOutUser(ChainOut, HasRefChainOutUses);

    int64_t CurrentOffset =
        Reference->getConstantOperandAPInt(1).getSExtValue();
    SDValue CurrentBase =
        CreateBasePlusOffset(Base, CurrentOffset, Cond, CondReg);

    unsigned RemainingPseudos = 0;
    for (unsigned i = 0, E = Pseudos.size(); i < E; ++i) {
      MachineSDNode *N = Pseudos[i];
      // First, copy as if not processed on this iteration
      Pseudos[RemainingPseudos++] = N;

      assert(N->getOperand(0) == Base);
      // First, perform simple checks for incompatibility
      const unsigned ThisNumOperands = N->getNumOperands();
      SDValue ThisCond = N->getOperand(ThisNumOperands - 3);
      SDValue ThisCondReg = N->getOperand(ThisNumOperands - 2);
      SDValue ThisChainIn = N->getOperand(ThisNumOperands - 1);
      if (ThisCond != Cond || ThisCondReg != CondReg || ThisChainIn != ChainIn)
        continue;

      const bool ThisIsLoad = isLoadNode(Subtarget, N);
      const EVT VecType =
          ThisIsLoad ? N->getValueType(0) : N->getOperand(3).getValueType();
      const unsigned AccessSize = VecType.getSizeInBits() / 8;

      // Then, perform the remaining check to ensure the nodes are identical
      // from the order point of view.
      // Reorder conservatively but ensure at least `Reference` is processed
      // on this iteration.
      SDValue ThisChainOut(N, ThisIsLoad ? 1 : 0);
      bool HasThisChainOutUses;
      SDNode *TheOnlyThisChainOutUser =
          getTheOnlyChainOutUser(ThisChainOut, HasThisChainOutUses);
      const bool HaveSingleAndIdenticalChainUsers =
          TheOnlyRefChainOutUser != nullptr &&
          TheOnlyRefChainOutUser == TheOnlyThisChainOutUser;
      const bool SafeToReorder = HaveSingleAndIdenticalChainUsers ||
                                 (!HasRefChainOutUses && !HasThisChainOutUses);
      if (Reference != N && !SafeToReorder)
        continue;

      // Finally, check that `N` fits into consequtive series of post-indexed
      // memory accesses.
      const int64_t ThisOffset = N->getConstantOperandAPInt(1).getSExtValue();
      if (ThisOffset != CurrentOffset)
        continue;
      CurrentOffset += AccessSize;

      Worklist.push_back(N);
      // No 'continue' occurred, N is successfully processed
      --RemainingPseudos;
    }
    Pseudos.resize(RemainingPseudos);

    LLVM_DEBUG(dbgs() << "ARM ISEL: Processing a chain of " << Worklist.size()
                      << " instructions\n");

    // On this iteration, make a chain of instructions from `Worklist`:
    // - prepare a copy of base address node, adjust it with the initial offset
    // - emit `Worklist.size() - 1` post-indexed instructions
    // - emit `Worklist.back()` as a no-offset load/store
    for (unsigned i = 0, E = Worklist.size() - 1; i < E; ++i) {
      CurrentBase = ReplacePseudoVLDST(Worklist[i], true, CurrentBase, Cond,
                                       CondReg, ChainIn);
    }
    ReplacePseudoVLDST(Worklist.back(), false, CurrentBase, Cond, CondReg,
                       ChainIn);
  }
}
