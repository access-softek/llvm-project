...
bool IRTranslator::translateKnownIntrinsic(const CallInst &CI, Intrinsic::ID ID,
  ...
  case Intrinsic::ptrauth_sign: {
    ...
    // In the degenerate case of a ptrauth_sign of a ptrauth global,
    // bail out and let this emit the sign we were asked for, rather than
    // producing an unrepresentable PtrAuthGlobalAddress(PtrAuthGlobalAddress)
    if (const GlobalVariable *BaseGV = dyn_cast<GlobalVariable>(BasePtr))
      if (BaseGV->getSection() == "llvm.ptrauth")
        return false;
