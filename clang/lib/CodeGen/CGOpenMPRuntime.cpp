...
ConstantAddress CGOpenMPRuntime::getAddrOfDeclareTargetVar(const VarDecl *VD) {
  ...
  if (Res && (*Res == OMPDeclareTargetDeclAttr::MT_Link ||
    ...
    llvm::Constant *Ptr = CGM.getModule().getNamedValue(PtrName);
    ...
    return ConstantAddress(Ptr, LlvmPtrTy, CGM.getContext().getDeclAlign(VD));
  }
  return ConstantAddress::invalid();
}
...
getPointerAndSize(CodeGenFunction &CGF, const Expr *E) {
  ...
  if (OASE) {
    ...
    llvm::Value *UpAddr = CGF.Builder.CreateConstGEP1_32(
        UpAddrAddress.getElementType(), UpAddrAddress.getRawPointer(CGF), /*Idx0=*/1);
    ...
    for (const auto *C : D.getClausesOfKind<OMPAffinityClause>()) {
      ...
      for (const Expr *E : C->varlists()) {
        ...
        LValue Base = CGF.MakeAddrLValue(
            CGF.Builder.CreateGEP(CGF, AffinitiesArray, Idx), KmpTaskAffinityInfoTy);
        // affs[i].base_addr = &<Affinities[i].second>;
        LValue BaseAddrLVal = CGF.EmitLValueForField(
            Base, *std::next(KmpAffinityInfoRD->field_begin(), BaseAddr));
        CGF.EmitStoreOfScalar(CGF.Builder.CreatePtrToInt(Addr, CGF.IntPtrTy),
...
void CGOpenMPRuntime::emitDestroyClause(CodeGenFunction &CGF, LValue DepobjLVal,
  ...
  getDependTypes(C, KmpDependInfoTy, FlagsTy);
  LValue Base = CGF.EmitLoadOfPointerLValue(
      DepobjLVal.getAddress(), C.VoidPtrTy.castAs<PointerType>());
  QualType KmpDependInfoPtrTy = C.getPointerType(KmpDependInfoTy);
...
void CGOpenMPRuntime::emitUpdateClause(CodeGenFunction &CGF, LValue DepobjLVal,
  ...
  llvm::Value *End = CGF.Builder.CreateGEP(
      Begin.getElementType(), Begin.getRawPointer(CGF), NumDeps);
  // The basic structure here is a while-do loop.
  ...
  ElementPHI->addIncoming(ElementNext.getRawPointer(CGF),
                          CGF.Builder.GetInsertBlock());
  llvm::Value *IsEmpty =
      CGF.Builder.CreateICmpEQ(ElementNext.getRawPointer(CGF), End,
		               "omp.isempty");
  CGF.Builder.CreateCondBr(IsEmpty, DoneBB, BodyBB);
  // Done.
  CGF.EmitBlock(DoneBB, /*IsFinished=*/true);
...
llvm::Value *CGOpenMPRuntime::emitTaskReductionInit(
  ...
  for (unsigned Cnt = 0; Cnt < Size; ++Cnt) {
    ...
    if (DelayedCreation) {
      ...
      CGF.EmitNullInitialization(FlagsLVal.getAddress(),
                                 FlagsLVal.getType());
  }
            ...
            for (const OMPClauseMappableExprCommon::MappableComponent &MC :
              ...
              if (const ValueDecl *VD = MC.getAssociatedDeclaration()) {
                ...
                Size = CGF.Builder.CreatePtrDiff(
                    CGF.Int8Ty, CGF.EmitCastToVoidPtr(ComponentLB.getRawPointer(CGF)),
                    CGF.EmitCastToVoidPtr(LB.getRawPointer(CGF)));
                break;
              }
            }
...
static void emitNonContiguousDescriptor(
  ...
  for (unsigned I = 0, L = 0, E = NonContigInfo.Dims.size(); I < E; ++I) {
    ...
    CGF.Builder.CreateStore(DAddr.getRawPointer(CGF), PAddr);
    ++L;
  }
}
...
static void emitOffloadingArrays(
  ...
  if (Info.NumberOfPtrs) {
    ...
    Info.RTArgs.MappersArray = MappersArray.getRawPointer(CGF);
    ...
    if (RuntimeSizes.all()) {
      ...
      if (RuntimeSizes.any()) {
        ...
        Info.RTArgs.SizesArray = Buffer.getRawPointer(CGF);
      } else {
