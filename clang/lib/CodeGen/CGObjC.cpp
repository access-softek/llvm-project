...
CodeGenFunction::EmitObjCBoxedExpr(const ObjCBoxedExpr *E) {
  ...
  if (ValueType->isObjCBoxableRecordType()) {
    ...
    llvm::Value *BitCast =
        Builder.CreateBitCast(Temporary.getRawPointer(*this), ConvertType(ArgQT));
...
static void emitStructGetterCall(CodeGenFunction &CGF, ObjCIvarDecl *ivar,
  ...
  llvm::Value *dest =
      CGF.Builder.CreateBitCast(CGF.ReturnValue.getRawPointer(CGF), CGF.VoidPtrTy);
  ...
  case PropertyImplStrategy::SetPropertyAndExpressionGet: {
    ...
    case TEK_Scalar: {
      ...
      if (propType->isReferenceType()) {
        value = LV.getAddress().getRawPointer(*this);
      } else {
...
static llvm::Value *emitARCLoadOperation(CodeGenFunction &CGF, Address addr,
  ...
  // Call the function.
  llvm::Value *result =
      CGF.EmitNounwindRuntimeCall(fn, addr.getRawPointer(CGF));
