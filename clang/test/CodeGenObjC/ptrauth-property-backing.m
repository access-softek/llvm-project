// RUN: %clang_cc1 -triple arm64-apple-ios -target-feature +pauth -fptrauth-calls -emit-llvm -fexceptions -fptrauth-intrinsics -o - %s | FileCheck %s
...
@interface Root {
  Class isa;
  void *__ptrauth(1, 1, 1, "authenticates-null-values") _field1;
  void *__ptrauth(1, 1, 1, "authenticates-null-values") _field2;
...
// CHECK: store atomic i64 [[RESULT]], ptr [[ADDR]] unordered

// CHECK-LABEL: define internal ptr @"\01-[Root field2]"
// CHECK: %ivar = load i32, ptr @"OBJC_IVAR_$_Root._field2"
...
// CHECK: store ptr [[RESULT]], ptr [[ADDR]]
...
// CHECK: %ivar = load i32, ptr @"OBJC_IVAR_$_Root._field4",
