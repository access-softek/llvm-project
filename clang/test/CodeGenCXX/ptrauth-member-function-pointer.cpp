// RUN: %clang_cc1 -triple arm64-apple-ios -target-feature +pauth -fptrauth-calls -fptrauth-intrinsics -emit-llvm -std=c++11 -O1 -disable-llvm-passes -o - %s | FileCheck -check-prefixes=CHECK,NODEBUG %s
// RUN: %clang_cc1 -triple arm64-apple-ios -target-feature +pauth -fptrauth-calls -fptrauth-intrinsics -emit-llvm -std=c++11 -O1 -disable-llvm-passes -debug-info-kind=limited -o - %s | FileCheck -check-prefixes=CHECK %s
// RUN: %clang_cc1 -triple arm64-apple-ios -target-feature +pauth -fptrauth-calls -fptrauth-intrinsics -emit-llvm -std=c++11 -O1 -disable-llvm-passes -stack-protector 1 -o - %s | FileCheck %s -check-prefix=STACK-PROT
// RUN: %clang_cc1 -triple arm64-apple-ios -target-feature +pauth -fptrauth-calls -fptrauth-intrinsics -emit-llvm -std=c++11 -O1 -disable-llvm-passes -stack-protector 2 -o - %s | FileCheck %s -check-prefix=STACK-PROT
// RUN: %clang_cc1 -triple arm64-apple-ios -target-feature +pauth -fptrauth-calls -fptrauth-intrinsics -emit-llvm -std=c++11 -O1 -disable-llvm-passes -stack-protector 3 -o - %s | FileCheck %s -check-prefix=STACK-PROT


// CHECK: %[[STRUCT_CLASS0:.*]] = type { { i64, i64 } }
...
// CHECK: @[[CONST_TEST_ARRAY_INIT_P0:.*]] = private unnamed_addr constant [1 x { i64, i64 }] [{ i64, i64 } { i64 ptrtoint (ptr @_ZN5Base011nonvirtual0Ev.ptrauth.1 to i64), i64 0 }], align 8
// CHECK: @[[CONST_TEST_ARRAY_INIT_P1:.*]] = private unnamed_addr constant [1 x { i64, i64 }] [{ i64, i64 } { i64 ptrtoint (ptr @_ZN5Base08virtual1Ev_vfpthunk_.ptrauth.2 to i64), i64 0 }], align 8
// CHECK: @[[CONST_TEST_ARRAY_INIT_C0:.*]] = private unnamed_addr constant %[[STRUCT_CLASS0]] { { i64, i64 } { i64 ptrtoint (ptr @_ZN5Base011nonvirtual0Ev.ptrauth.1 to i64), i64 0 } }, align 8
// CHECK: @[[CONST_TEST_ARRAY_INIT_C1:.*]] = private unnamed_addr constant %[[STRUCT_CLASS0]] { { i64, i64 } { i64 ptrtoint (ptr @_ZN5Base08virtual1Ev_vfpthunk_.ptrauth.2 to i64), i64 0 } }, align 8
...
void test_builtin_ptrauth_type_discriminator() {
  ...
  d = __builtin_ptrauth_type_discriminator(void (Base0::* const)());
  typedef void (Base0::* const ConstMFP)();
  typedef ConstMFP __attribute__((address_space(1))) AddrSpaceMFP;
  d = __builtin_ptrauth_type_discriminator(ConstMFP);
  d = __builtin_ptrauth_type_discriminator(AddrSpaceMFP);
}
...
// CHECK: call void @llvm.memcpy.p0.p0.i64(ptr align 8 %{{.*}}, ptr align 8 @[[CONST_TEST_ARRAY_INIT_P0]], i64 16, i1 false)
// CHECK: call void @llvm.memcpy.p0.p0.i64(ptr align 8 %{{.*}}, ptr align 8 @[[CONST_TEST_ARRAY_INIT_P1]], i64 16, i1 false)
// CHECK: call void @llvm.memcpy.p0.p0.i64(ptr align 8 %{{.*}}, ptr align 8 @[[CONST_TEST_ARRAY_INIT_C0]], i64 16, i1 false)
// CHECK: call void @llvm.memcpy.p0.p0.i64(ptr align 8 %{{.*}}, ptr align 8 @[[CONST_TEST_ARRAY_INIT_C1]], i64 16, i1 false)
// CHECK: store { i64, i64 } { i64 ptrtoint (ptr @_ZN5Base011nonvirtual0Ev.ptrauth.1 to i64), i64 0 }, ptr %{{.*}}, align 8
// CHECK: store { i64, i64 } { i64 ptrtoint (ptr @_ZN5Base08virtual1Ev_vfpthunk_.ptrauth.2 to i64), i64 0 }, ptr %{{.*}}, align 8
