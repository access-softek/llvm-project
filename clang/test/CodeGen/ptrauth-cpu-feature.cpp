...
// UN: %clang %s -S -o - -target aarch64 -mcpu=apple-a12        -mbranch-protection=pauthabi 2>&1 \
// UN:     | FileCheck %s --check-prefix=PAUTH --implicit-check-not=error --implicit-check-not=warning
// UN: %clang %s -S -o - -target aarch64 -march=armv8.3-a       -mbranch-protection=pauthabi 2>&1 \
// UN:     | FileCheck %s --check-prefix=PAUTH --implicit-check-not=error --implicit-check-not=warning
// UN: %clang %s -S -o - -target aarch64 -march=armv9-a         -mbranch-protection=pauthabi 2>&1 \
// UN:     | FileCheck %s --check-prefix=PAUTH --implicit-check-not=error --implicit-check-not=warning
// UN: %clang %s -S -o - -target aarch64 -march=armv8.2-a+pauth -mbranch-protection=pauthabi 2>&1 \
// UN:     | FileCheck %s --check-prefix=PAUTH --implicit-check-not=error --implicit-check-not=warning
...
//// TODO!
// UN: %clang %s -S -o - -target aarch64 -march=armv8.3-a -fptrauth-init-fini -fptrauth-calls              2>&1 \
// UN:     | FileCheck %s --check-prefix=NO-UNEXPECTED
// UN: %clang %s -S -o - -target aarch64 -march=armv8.3-a -fptrauth-init-fini -mbranch-protection=pauthabi 2>&1 \
// UN:     | FileCheck %s --check-prefix=NO-UNEXPECTED
...
//// TODO!
// UN:     %clang %s -S -o - -target arm64e-apple-ios                  2>&1 \
// UN:     | FileCheck %s --check-prefix=PAUTH
// UN: not %clang %s -S -o - -target arm64e-apple-ios -mcpu=cortex-a72 2>&1 \
// UN:     | FileCheck %s --check-prefixes=FAIL-INTRINSICS,FAIL-CALLS,FAIL-RETURNS,NO-UNEXPECTED
...
// FAIL-INTRINSICS: error: -fptrauth-intrinsics or -mbranch-protection=pauthabi is passed that require enabling PAuth CPU feature
// FAIL-CALLS:      error: -fptrauth-calls or -mbranch-protection=pauthabi is passed that require enabling PAuth CPU feature
// FAIL-RETURNS:    error: -fptrauth-returns or -mbranch-protection=pauthabi is passed that require enabling PAuth CPU feature
