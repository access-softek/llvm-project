...
__INTPTR_TYPE__ test_read_globals() {
  ...
  // CHECK: resign.cont10:
  // CHECK: [[ADDR:%.*]] = load i64, ptr getelementptr inbounds (%struct.A, ptr @gs1, i32 0, i32 1
  // CHECK: resign.nonnull11:
  // CHECK: [[AUTHED:%.*]] = call i64 @llvm.ptrauth.auth(i64 [[ADDR]], i32 1, i64 9182)
  // CHECK: resign.cont12: 
  // CHECK: [[ADDR:%.*]] = load i64, ptr getelementptr inbounds (%struct.A, ptr @gs1, i32 0, i32 2)
  ...
  // CHECK: [[ADDR:%.*]] = load i64, ptr getelementptr inbounds (%struct.B, ptr @gs2, i32 0, i32 1)
  // CHECK: [[BLENDED:%.*]] = call i64 @llvm.ptrauth.blend(i64 ptrtoint (ptr getelementptr inbounds (%struct.B, ptr @gs2, i32 0, i32 1) to i64), i64 23674)
  // CHECK: [[AUTHED:%.*]] = call i64 @llvm.ptrauth.auth(i64 [[ADDR]], i32 1, i64 [[BLENDED]])
  // CHECK: [[ADDR:%.*]] = load i64, ptr getelementptr inbounds (%struct.B, ptr @gs2, i32 0, i32 2)
  // CHECK: [[BLENDED:%.*]] = call i64 @llvm.ptrauth.blend(i64 ptrtoint (ptr getelementptr inbounds (%struct.B, ptr @gs2, i32 0, i32 2) to i64), i64 163)
...
void test_set_A(struct A *a, __INTPTR_TYPE__ x, int y) {
  ...
  // CHECK: [[F2:%.*]] = getelementptr inbounds %struct.A, ptr [[A]], i32 0, i32 2
...
void test_set_B(struct B *b, __INTPTR_TYPE__ x, int y) {
  ...
  // CHECK: [[F1_ADDR:%.*]] = getelementptr inbounds %struct.B, ptr [[B]], i32 0, i32 1
  ...
  b->f2 = 0;
  // CHECK: [[B:%.*]] = load ptr, ptr %b.addr
  // CHECK: [[F2_ADDR:%.*]] = getelementptr inbounds %struct.B, ptr [[B]], i32 0, i32 2
...
__INTPTR_TYPE__ test_get_A(struct A *a) {
  ...
  // CHECK: [[A:%.*]] = load ptr, ptr %a.addr
  // CHECK: [[F0_ADDR:%.*]] = getelementptr inbounds %struct.A, ptr [[A]], i32 0, i32 0
  // CHECK: [[F0:%.*]] = load i64, ptr [[F0_ADDR]]
  // CHECK: [[AUTH:%.*]] = call i64 @llvm.ptrauth.auth(i64 [[F0]], i32 1, i64 431)
  // CHECK: [[A:%.*]] = load ptr, ptr %a.addr
  // CHECK: [[F1_ADDR:%.*]] = getelementptr inbounds %struct.A, ptr [[A]], i32 0, i32 1
  // CHECK: [[F1:%.*]] = load i64, ptr [[F1_ADDR]]
  // CHECK: [[AUTH:%.*]] = call i64 @llvm.ptrauth.auth(i64 [[F1]], i32 1, i64 9182)
  // CHECK: [[A:%.*]] = load ptr, ptr %a.addr
  // CHECK: [[F2_ADDR:%.*]] = getelementptr inbounds %struct.A, ptr [[A]], i32 0, i32 2
...
__INTPTR_TYPE__ test_get_B(struct B *b) {
  ...
  // CHECK: [[B:%.*]] = load ptr, ptr %b.addr
  // CHECK: [[F0:%.*]] = getelementptr inbounds %struct.B, ptr [[B]], i32 0, i32 0
  ...
  // CHECK: [[AUTHED:%.*]] = call i64 @llvm.ptrauth.auth(i64 [[VALUE]], i32 1, i64 [[BLENDED]])
  // CHECK: [[B:%.*]] = load ptr, ptr %b.addr
  // CHECK: [[ADDR:%.*]] = getelementptr inbounds %struct.B, ptr [[B]], i32 0, i32 1
  // CHECK: [[VALUE:%.*]] = load i64, ptr [[ADDR]]
  // CHECK: [[CAST_ADDR:%.*]] = ptrtoint ptr [[ADDR]] to i64
  // CHECK: [[BLENDED:%.*]] = call i64 @llvm.ptrauth.blend(i64 [[CAST_ADDR]], i64 23674)
  // CHECK: [[AUTHED:%.*]] = call i64 @llvm.ptrauth.auth(i64 [[VALUE]], i32 1, i64 [[BLENDED]])
  // CHECK: [[B:%.*]] = load ptr, ptr %b.addr
  // CHECK: [[ADDR:%.*]] = getelementptr inbounds %struct.B, ptr [[B]], i32 0, i32 2
  // CHECK: [[VALUE:%.*]] = load i64, ptr [[ADDR]]
  // CHECK: [[CAST_ADDR:%.*]] = ptrtoint ptr [[ADDR]] to i64
...
void test_resign(struct A* a, const struct B *b) {
  ...
  // CHECK: [[F0:%.*]] = getelementptr inbounds %struct.A, ptr [[A]], i32 0, i32 0
  // CHECK: [[B:%.*]] = load ptr, ptr %b.addr, align 8
  // CHECK: [[F01:%.*]] = getelementptr inbounds %struct.B, ptr [[B]], i32 0, i32 0
