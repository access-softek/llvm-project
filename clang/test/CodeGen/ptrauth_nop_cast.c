// RUN: %clang_cc1 -no-opaque-pointers -triple arm64e-apple-ios13.0 -fptrauth-calls -fptrauth-intrinsics -O1 %s -emit-llvm -o - | FileCheck %s

#include <ptrauth.h>

void *test_nop_cast(void (*fptr)(int)) {
  // CHECK: define i8* @test_nop_cast(void (i32)* noundef readnone [[FPTR:%.*]])
...
VoidFn test_nop_cast_functype(void (*fptr)(int, int)) {
  // CHECK: define void ()* @test_nop_cast_functype(void (i32, i32)* noundef readnone [[FPTR:%.*]])
...
void *test_nop_cast_direct() {
  // CHECK: define nonnull i8* @test_nop_cast_direct()
