// RUN: %clang_cc1 -triple arm64-apple-ios   -target-feature +pauth -fptrauth-calls -fptrauth-intrinsics -emit-llvm %s  -o - | FileCheck -check-prefix=CHECK %s
// RUN: %clang_cc1 -triple aarch64-linux-gnu -target-feature +pauth -fptrauth-calls -fptrauth-intrinsics -emit-llvm %s  -o - | FileCheck -check-prefix=CHECK %s
