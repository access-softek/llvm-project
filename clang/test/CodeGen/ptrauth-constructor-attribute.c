...
// RUN: %clang -target aarch64-elf -march=armv8.3-a+pauth -fptrauth-init-fini    -S -emit-llvm -o - -c %s | FileCheck --check-prefix=SIGNED %s
// RUN: %clang -target aarch64-elf -march=armv8.3-a+pauth -fno-ptrauth-init-fini -S -emit-llvm -o - -c %s | FileCheck --check-prefix=UNSIGNED %s
