...
// RUN: %clang -fno-ptrauth-abi-version                                        -target arm64e-apple-ios -c %s -### 2>&1 | FileCheck %s --check-prefix NOABIVERSION --check-prefix NOKERNELABIVERSION
// RUN: %clang -fptrauth-abi-version=5         -fno-ptrauth-abi-version        -target arm64e-apple-ios -c %s -### 2>&1 | FileCheck %s --check-prefix NOABIVERSION --check-prefix NOKERNELABIVERSION
// RUN: %clang -fno-ptrauth-abi-version        -fptrauth-abi-version=5         -target arm64e-apple-ios -c %s -### 2>&1 | FileCheck %s --check-prefix ABIVERSION --check-prefix NOKERNELABIVERSION
