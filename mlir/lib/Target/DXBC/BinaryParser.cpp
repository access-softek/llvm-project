...
public:
  ...
  dxbc::InlineOperandAttr buildInlineOperandAttr(
      dxbc::InlineOperandType operandType, uint32_t components,
      std::optional<dxbc::ComponentMask> mask, ArrayRef<int64_t> indexArray) {
    auto *ctx = builder.getContext();
    auto maskAttr = mask ? dxbc::ComponentMaskAttr::get(ctx, *mask)
                         : dxbc::ComponentMaskAttr();
    auto indexAttr = indexArray.empty()
                         ? DenseI64ArrayAttr()
                         : DenseI64ArrayAttr::get(ctx, indexArray);
    return dxbc::InlineOperandAttr::get(ctx, operandType, components, maskAttr,
                                        indexAttr);
  }
  ...
  Instruction buildDclInput(dxbc::InlineOperandAttr operand, Location loc) {
    return dxbc::DclInput::create(builder, loc, operand);
  }

  Instruction buildDclOutput(dxbc::InlineOperandAttr operand, Location loc) {
  ...
  Instruction buildDclInputSgv(dxbc::InlineOperandAttr operand,
  ...
  Instruction buildDclInputSiv(dxbc::InlineOperandAttr operand,
                               dxbc::SystemValueName name, Location loc) {
    auto nameAttr = dxbc::SystemValueNameAttr::get(builder.getContext(), name);
    return dxbc::DclInputSiv::create(builder, loc, operand, nameAttr);
  }

  Instruction buildDclOutputSgv(dxbc::InlineOperandAttr operand,
                                dxbc::SystemValueName name, Location loc) {
    auto nameAttr = dxbc::SystemValueNameAttr::get(builder.getContext(), name);
  ...
  Instruction buildDclTgsmRaw(dxbc::InlineOperandAttr operand,
                              uint32_t byteCount, Location loc) {
    return dxbc::DclTgsmRaw::create(builder, loc, operand,
                                    builder.getI32IntegerAttr(byteCount));
  }

  Instruction buildDclTgsmStructured(dxbc::InlineOperandAttr operand,
...
public:
  ...
  FailureOr<Instruction> parseDclStream(Location loc) {
    ...
    if (operand->getType() != dxbc::InlineOperandType::stream)
  ...
  FailureOr<dxbc::InlineOperandAttr> parseInlineOperand() {
    auto token = parseToken();
    ...
    auto type = dxbc::symbolizeInlineOperandType(rawOperandType);
    if (!type)
    ...
    std::optional<dxbc::ComponentMask> mask;
    if (components->kind == OperandComponentsKind::Mask)
  ...
  FailureOr<Instruction> parseDclSampler(uint32_t opcodeToken, Location loc) {
    ...
    auto operand = parseInlineOperand();
    FAILURE_IF_FAILED(operand);
    if (operand->getType() != dxbc::InlineOperandType::sampler)
  ...
  FailureOr<Instruction> parseDclResource(uint32_t opcodeToken, Location loc) {
    ...
    auto operand = parseInlineOperand();
    FAILURE_IF_FAILED(operand);
    if (operand->getType() != dxbc::InlineOperandType::resource)
