...
public:
  ...
  Instruction buildDclStream(uint32_t index, Location loc) {
    return dxbc::DclStream::create(builder, loc,
  ...
  Instruction buildUnaryOp(dxbc::DstOperandAttr dst, dxbc::SrcOperandAttr src,
                           uint32_t preciseMask, Location loc) {
    auto preciseAttr =
        preciseMask
            ? dxbc::ComponentMaskAttr::get(
                  context, static_cast<dxbc::ComponentMask>(preciseMask))
            : dxbc::ComponentMaskAttr();
    return OpT::create(builder, loc, dst, src, preciseAttr);
  }

  template <typename OpT>
  Instruction buildBinaryOp(dxbc::DstOperandAttr dst, dxbc::SrcOperandAttr lhs,
                            dxbc::SrcOperandAttr rhs, uint32_t preciseMask,
                            Location loc) {
  ...
  template <typename OpT>
  Instruction
  buildMultiplyAddOp(dxbc::DstOperandAttr dst, dxbc::SrcOperandAttr lhs,
                     dxbc::SrcOperandAttr rhs, dxbc::SrcOperandAttr acc,
                     uint32_t preciseMask, Location loc) {
  ...
  template <typename OpT>
  Instruction buildSincosOp(dxbc::DstOperandAttr sin, dxbc::DstOperandAttr cos,
                            dxbc::SrcOperandAttr operand, uint32_t preciseMask,
                            Location loc) {
...
public:
  ...
  getRequiredImmIndices(dxbc::DstOperandAttr operand, Location loc) {
...
#define UNARY_OP(OP)                                                           \
  decodeUnaryOp<dxbc::OP>(beginOffset, instructionLengthInTokens,              \
                          modifier.preciseMask, getLocation())
...
#define BINARY_OP(OP)                                                          \
  decodeBinaryOp<dxbc::OP>(beginOffset, instructionLengthInTokens,             \
    ...
    case D3D10_SB_OPCODE_MAD:
      return decodeSaturableMultiplyAddOp<dxbc::Mad, dxbc::MadSat>(
          beginOffset, instructionLengthInTokens, modifier.saturate,
          modifier.preciseMask, getLocation());
    ...
    case D3D10_SB_OPCODE_SINCOS:
      return decodeSaturableSincosOp<dxbc::Sincos, dxbc::SincosSat>(
          beginOffset, instructionLengthInTokens, modifier.saturate,
          modifier.preciseMask, getLocation());
