//===----------- BinaryParser.cpp - Parse DXBC binary to MLIR -------------===//
//
...
#define DEBUG_TYPE "import-dxbc-bin"
...
static dxbc::ComponentMask decodeComponentMask(uint32_t rawComponentMask) {
  auto componentMask = static_cast<dxbc::ComponentMask>(0);
  if (rawComponentMask & D3D10_SB_OPERAND_4_COMPONENT_MASK_X)
    componentMask |= dxbc::ComponentMask::x;
  if (rawComponentMask & D3D10_SB_OPERAND_4_COMPONENT_MASK_Y)
    componentMask |= dxbc::ComponentMask::y;
  if (rawComponentMask & D3D10_SB_OPERAND_4_COMPONENT_MASK_Z)
    componentMask |= dxbc::ComponentMask::z;
  if (rawComponentMask & D3D10_SB_OPERAND_4_COMPONENT_MASK_W)
    componentMask |= dxbc::ComponentMask::w;
...
public:
  ...
  using Module = dxbc::ModuleOp;

  Module createModule(dxbc::ProgramTypeAttr programType,
  ...
  Index buildIndexImm32(int32_t imm, FileLineColLoc loc) {
    Operation *op =
        dxbc::IndexImm::create(builder, loc, builder.getType<dxbc::IndexType>(),
  ...
  Index buildIndexRelative(Operand operand, FileLineColLoc loc) {
    Operation *op = dxbc::IndexRel::create(
        builder, loc, builder.getType<dxbc::IndexType>(), operand);
    return op->getResults()[0];
  }

  Index buildIndexImm32PlusRelative(int32_t imm, Operand operand,
                                    FileLineColLoc loc) {
    Operation *op = dxbc::IndexRelImm::create(
        builder, loc, builder.getType<dxbc::IndexType>(), operand,
  ...
  Operand buildOperandImm32(ArrayRef<int32_t> values, FileLineColLoc loc) {
    Operation *op = dxbc::OperandImm::create(
        builder, loc, builder.getType<dxbc::LegacyOperandType>(),
  ...
  Operand buildOperand(uint32_t opType, const OperandComponents &components,
    ...
    Operation *op = dxbc::Operand::create(
        builder, loc, builder.getType<dxbc::LegacyOperandType>(), indices,
        attrs);
    return op->getResults()[0];
  }
  ...
  Instruction buildInstruction(StringRef name, ArrayRef<Operand> operands,
                               ...
                               FileLineColLoc loc) {
    return dxbc::Instruction::create(builder, loc, operands,
  ...
  Instruction buildUnknown(ArrayRef<uint32_t> tokens, Location loc) {
    ...
    return dxbc::Unknown::create(
        builder, loc,
        DenseI32ArrayAttr::get(builder.getContext(), signedTokens));
  }

  Instruction buildDclGlobalFlags(dxbc::GlobalFlags flags, Location loc) {
    auto flagsAttr = dxbc::GlobalFlagsAttr::get(builder.getContext(), flags);
  ...
  Instruction buildDclInputControlPointCount(uint32_t count, Location loc) {
    return dxbc::DclInputControlPointCount::create(
        builder, loc, builder.getI32IntegerAttr(count));
  }

  Instruction buildDclOutputControlPointCount(uint32_t count, Location loc) {
    return dxbc::DclOutputControlPointCount::create(
        builder, loc, builder.getI32IntegerAttr(count));
  }

  Instruction buildDclTessellatorDomain(dxbc::TessellatorDomain domain,
                                        Location loc) {
    auto domainAttr =
        dxbc::TessellatorDomainAttr::get(builder.getContext(), domain);
    return dxbc::DclTessellatorDomain::create(builder, loc, domainAttr);
  }

  Instruction buildDclTessellatorOutputPrimitive(
      dxbc::TessellatorOutputPrimitiveType outputPrimitiveType, Location loc) {
    auto outputPrimitiveTypeAttr =
        dxbc::TessellatorOutputPrimitiveTypeAttr::get(builder.getContext(),
                                                      outputPrimitiveType);
    return dxbc::DclTessellatorOutputPrimitive::create(builder, loc,
                                                       outputPrimitiveTypeAttr);
  }

  Instruction
  buildDclOutputTopology(dxbc::OutputPrimitiveTopology outputTopology,
                         Location loc) {
    auto outputTopologyAttr = dxbc::OutputPrimitiveTopologyAttr::get(
        builder.getContext(), outputTopology);
    return dxbc::DclOutputTopology::create(builder, loc, outputTopologyAttr);
  }

  Instruction buildDclTessellatorPartitioning(
      dxbc::TessellatorPartitioningMode partitioningMode, Location loc) {
    auto partitioningModeAttr = dxbc::TessellatorPartitioningModeAttr::get(
        builder.getContext(), partitioningMode);
    return dxbc::DclTessellatorPartitioning::create(builder, loc,
                                                    partitioningModeAttr);
  }

  Instruction buildDclInputPrimitive(dxbc::InputPrimitive inputPrimitive,
                                     Location loc) {
    auto inputPrimitiveAttr =
        dxbc::InputPrimitiveAttr::get(builder.getContext(), inputPrimitive);
  ...
  Instruction buildDclGsInstanceCount(uint32_t count, Location loc) {
    return dxbc::DclGsInstanceCount::create(builder, loc,
                                            builder.getI32IntegerAttr(count));
  }

  Instruction buildDclMaxOutputVertexCount(uint32_t count, Location loc) {
    return dxbc::DclMaxOutputVertexCount::create(builder, loc, count);
  }
  ...
  Instruction buildDclInputPs(dxbc::InterpolationMode interpolationMode,
                              dxbc::DstOperandAttr operand, Location loc) {
  ...
  Instruction buildDclInputPsSgv(dxbc::DstOperandAttr operand,
                                 dxbc::SystemValueName systemValueName,
                                 Location loc) {
    auto systemValueNameAttr =
        dxbc::SystemValueNameAttr::get(builder.getContext(), systemValueName);
  ...
  dxbc::DstOperandAttr
  buildDstOperandAttr(dxbc::OperandType operandType,
                      const OperandComponents &components,
                      ArrayRef<dxbc::IndexAttr> indexEntries,
                      std::optional<OperandModifier> opModifier) {
    auto componentsValue = components.num == 0 ? dxbc::OperandComponents::none
                           : components.num == 1
                               ? dxbc::OperandComponents::scalar
                               : dxbc::OperandComponents::vector;
    auto componentsAttr =
        dxbc::OperandComponentsAttr::get(context, componentsValue);

    dxbc::ComponentMaskAttr maskAttr;
    if (components.kind == OperandComponentsKind::Mask)
      maskAttr = dxbc::ComponentMaskAttr::get(
          context, decodeComponentMask(components.mask));

    dxbc::OperandIndexAttr indexAttr;
    if (!indexEntries.empty())
      indexAttr = dxbc::OperandIndexAttr::get(context, indexEntries);

    dxbc::OperandMinPrecisionAttr minPrecisionAttr;
    if (opModifier && opModifier->minPrecision != 0) {
      if (auto p = dxbc::symbolizeOperandMinPrecision(opModifier->minPrecision))
  ...
  dxbc::SrcOperandAttr
  buildSrcOperandAttr(dxbc::OperandType operandType,
                      const OperandComponents &components,
                      ArrayRef<dxbc::IndexAttr> indexEntries,
                      std::optional<OperandModifier> opModifier,
    ...
    if (components.kind == OperandComponentsKind::Swizzle) {
      ...
      swizzleAttr = dxbc::SwizzleAttr::get(context, swizzleComponents);
    } else if (components.kind == OperandComponentsKind::One) {
      swizzleAttr = dxbc::SwizzleAttr::get(
    ...
    dxbc::OperandModifierAttr modifierAttr;
    dxbc::OperandMinPrecisionAttr minPrecisionAttr;
    UnitAttr nonUniformAttr;
    if (opModifier) {
      if (opModifier->modifier != 0) {
        if (auto m = dxbc::symbolizeOperandModifier(opModifier->modifier))
          modifierAttr = dxbc::OperandModifierAttr::get(context, *m);
      }
      if (opModifier->minPrecision != 0) {
        if (auto p =
                dxbc::symbolizeOperandMinPrecision(opModifier->minPrecision))
          minPrecisionAttr = dxbc::OperandMinPrecisionAttr::get(context, *p);
      }
    ...
    return dxbc::SrcOperandAttr::get(
  ...
  dxbc::IndexAttr buildOperandIndexImm32(int32_t imm) {
    return dxbc::IndexAttr::get(context, builder.getI32IntegerAttr(imm),
                                dxbc::SrcOperandAttr());
  }

  dxbc::IndexAttr buildOperandIndexImm64(int64_t imm) {
    return dxbc::IndexAttr::get(context, builder.getI64IntegerAttr(imm),
                                dxbc::SrcOperandAttr());
  }

  dxbc::IndexAttr buildOperandIndexRelative(dxbc::SrcOperandAttr relative) {
    return dxbc::IndexAttr::get(context, IntegerAttr(), relative);
  }

  dxbc::IndexAttr
  buildOperandIndexImm32PlusRelative(int32_t imm,
                                     dxbc::SrcOperandAttr relative) {
    return dxbc::IndexAttr::get(context, builder.getI32IntegerAttr(imm),
                                relative);
  }
  ...
  dxbc::ComponentMaskAttr buildPreciseAttr(uint32_t preciseMask) {
    if (!preciseMask)
      return dxbc::ComponentMaskAttr();
    return dxbc::ComponentMaskAttr::get(
        context, static_cast<dxbc::ComponentMask>(preciseMask));
  }
  ...
  buildOp(uint32_t preciseMask, Location loc,
          const std::array<dxbc::DstOperandAttr, NumDstOperands> &dsts,
          const std::array<dxbc::SrcOperandAttr, NumSrcOperands> &srcs) {
    return std::apply(
        [&](auto... dstOperands) {
          return std::apply(
  ...
  Instruction buildDclInput(dxbc::DstOperandAttr operand, Location loc) {
    return dxbc::DclInput::create(builder, loc, operand);
  }

  Instruction buildDclOutput(dxbc::DstOperandAttr operand, Location loc) {
  ...
  Instruction buildDclInputSgv(dxbc::DstOperandAttr operand,
                               dxbc::SystemValueName name, Location loc) {
    auto nameAttr = dxbc::SystemValueNameAttr::get(builder.getContext(), name);
    return dxbc::DclInputSgv::create(builder, loc, operand, nameAttr);
  }

  Instruction buildDclInputSiv(dxbc::DstOperandAttr operand,
                               dxbc::SystemValueName name, Location loc) {
    auto nameAttr = dxbc::SystemValueNameAttr::get(builder.getContext(), name);
    return dxbc::DclInputSiv::create(builder, loc, operand, nameAttr);
  }

  Instruction buildDclOutputSgv(dxbc::DstOperandAttr operand,
                                dxbc::SystemValueName name, Location loc) {
    auto nameAttr = dxbc::SystemValueNameAttr::get(builder.getContext(), name);
    return dxbc::DclOutputSgv::create(builder, loc, operand, nameAttr);
  }

  Instruction buildDclOutputSiv(dxbc::DstOperandAttr operand,
                                dxbc::SystemValueName name, Location loc) {
    auto nameAttr = dxbc::SystemValueNameAttr::get(builder.getContext(), name);
  ...
  Instruction buildDclHsMaxTessFactor(float maxTessFactor, Location loc) {
    return dxbc::DclHsMaxTessFactor::create(
  ...
  Instruction buildDclHsJoinPhaseInstanceCount(uint32_t count, Location loc) {
    return dxbc::DclHsJoinPhaseInstanceCount::create(
        builder, loc, builder.getUI32IntegerAttr(count));
  }

  Instruction buildDclHsForkPhaseInstanceCount(uint32_t count, Location loc) {
    return dxbc::DclHsForkPhaseInstanceCount::create(
        builder, loc, builder.getUI32IntegerAttr(count));
  }

  Instruction buildDclTgsmRaw(dxbc::DstOperandAttr operand, uint32_t byteCount,
                              Location loc) {
    return dxbc::DclTgsmRaw::create(builder, loc, operand,
                                    builder.getI32IntegerAttr(byteCount));
  }

  Instruction buildDclTgsmStructured(dxbc::DstOperandAttr operand,
                                     uint32_t structByteStride,
                                     uint32_t structCount, Location loc) {
    return dxbc::DclTgsmStructured::create(
  ...
  Instruction buildDclImmediateConstantBuffer(ArrayRef<float> values,
                                              Location loc) {
    return dxbc::DclImmediateConstantBuffer::create(builder, loc, values);
  }

  Instruction buildDclSampler(uint32_t id, std::optional<uint32_t> lbound,
                              std::optional<uint32_t> ubound,
                              std::optional<uint32_t> space,
                              dxbc::SamplerMode mode, Location loc) {
    auto optionalToAttr = [&](std::optional<uint32_t> v) -> IntegerAttr {
      return v ? builder.getI32IntegerAttr(*v) : IntegerAttr();
    };
    return dxbc::DclSampler::create(
  ...
  Instruction buildDclResource(
      ...
      dxbc::ResourceReturnType w, std::optional<uint32_t> sampleCount,
    ...
    return dxbc::DclResource::create(builder, loc, id, dim, x, y, z, w,
  ...
  Instruction buildDclResourceStructured(uint32_t id, uint32_t structByteStride,
    ...
    return dxbc::DclResourceStructured::create(builder, loc, id,
  ...
  Instruction buildDclResourceRaw(uint32_t id, std::optional<uint32_t> lbound,
    ...
    return dxbc::DclResourceRaw::create(builder, loc, id, toAttr(lbound),
                                        toAttr(ubound), toAttr(space));
  }
  ...
  Instruction buildGather4(dxbc::DstOperandAttr dst,
                           dxbc::SrcOperandAttr srcAddress,
                           dxbc::SrcOperandAttr srcResource,
                           dxbc::SrcOperandAttr srcSampler,
                           dxbc::SampleOffsetAttr offset, Location loc) {
    return dxbc::Gather4::create(builder, loc, dst, srcAddress, srcResource,
                                 srcSampler, offset);
  }

  Instruction buildGather4Feedback(dxbc::DstOperandAttr dst,
                                   dxbc::SrcOperandAttr srcAddress,
                                   dxbc::SrcOperandAttr srcResource,
                                   dxbc::SrcOperandAttr srcSampler,
                                   dxbc::DstOperandAttr feedback,
                                   ...
                                   Location loc) {
    return dxbc::Gather4Feedback::create(builder, loc, dst, srcAddress,
                                         srcResource, srcSampler, feedback,
                                         offset);
  }

  Instruction buildGather4C(dxbc::DstOperandAttr dst,
                            dxbc::SrcOperandAttr srcAddress,
                            dxbc::SrcOperandAttr srcResource,
                            dxbc::SrcOperandAttr srcSampler,
                            dxbc::SrcOperandAttr srcReferenceValue,
                            dxbc::SampleOffsetAttr offset, Location loc) {
    return dxbc::Gather4C::create(builder, loc, dst, srcAddress, srcResource,
                                  srcSampler, srcReferenceValue, offset);
  }
  ...
  Instruction buildGather4CFeedback(
      dxbc::DstOperandAttr dst, dxbc::SrcOperandAttr srcAddress,
      dxbc::SrcOperandAttr srcResource, dxbc::SrcOperandAttr srcSampler,
      dxbc::SrcOperandAttr srcReferenceValue, dxbc::DstOperandAttr feedback,
      dxbc::SampleOffsetAttr offset, Location loc) {
    return dxbc::Gather4CFeedback::create(builder, loc, dst, srcAddress,
                                          srcResource, srcSampler,
                                          srcReferenceValue, feedback, offset);
  }

  Instruction buildGather4PO(dxbc::DstOperandAttr dst,
                             dxbc::SrcOperandAttr srcAddress,
                             dxbc::SrcOperandAttr srcOffset,
                             dxbc::SrcOperandAttr srcResource,
                             dxbc::SrcOperandAttr srcSampler, Location loc) {
    return dxbc::Gather4PO::create(builder, loc, dst, srcAddress, srcOffset,
                                   srcResource, srcSampler);
  }

  Instruction buildGather4POFeedback(dxbc::DstOperandAttr dst,
                                     dxbc::SrcOperandAttr srcAddress,
                                     dxbc::SrcOperandAttr srcOffset,
                                     dxbc::SrcOperandAttr srcResource,
                                     dxbc::SrcOperandAttr srcSampler,
                                     dxbc::DstOperandAttr feedback,
                                     Location loc) {
    return dxbc::Gather4POFeedback::create(builder, loc, dst, srcAddress,
                                           srcOffset, srcResource, srcSampler,
                                           feedback);
  }

  Instruction buildGather4POC(dxbc::DstOperandAttr dst,
                              dxbc::SrcOperandAttr srcAddress,
                              dxbc::SrcOperandAttr srcOffset,
                              dxbc::SrcOperandAttr srcResource,
                              dxbc::SrcOperandAttr srcSampler,
                              dxbc::SrcOperandAttr srcReferenceValue,
                              Location loc) {
    return dxbc::Gather4POC::create(builder, loc, dst, srcAddress, srcOffset,
                                    srcResource, srcSampler, srcReferenceValue);
  }

  Instruction buildGather4POCFeedback(
      dxbc::DstOperandAttr dst, dxbc::SrcOperandAttr srcAddress,
      dxbc::SrcOperandAttr srcOffset, dxbc::SrcOperandAttr srcResource,
      dxbc::SrcOperandAttr srcSampler, dxbc::SrcOperandAttr srcReferenceValue,
      dxbc::DstOperandAttr feedback, Location loc) {
    return dxbc::Gather4POCFeedback::create(builder, loc, dst, srcAddress,
                                            srcOffset, srcResource, srcSampler,
...
public:
  ...
  FailureOr<Instruction> parseDclTessellatorDomain(uint32_t opcodeToken,
    ...
    auto domain = dxbc::symbolizeTessellatorDomain(rawDomain);
    if (!domain)
  ...
  parseDclTessellatorOutputPrimitive(uint32_t opcodeToken, Location loc) {
    ...
    auto outputPrimitiveType =
        dxbc::symbolizeTessellatorOutputPrimitiveType(rawOutputPrimitiveType);
  ...
  FailureOr<Instruction> parseDclOutputTopology(uint32_t opcodeToken,
    ...
    auto outputTopology =
        dxbc::symbolizeOutputPrimitiveTopology(rawOutputTopology);
    if (!outputTopology)
  ...
  FailureOr<Instruction> parseDclTessellatorPartitioning(uint32_t opcodeToken,
    ...
    auto partitioningMode =
        dxbc::symbolizeTessellatorPartitioningMode(rawPartitioningMode);
    if (!partitioningMode)
  ...
  FailureOr<Instruction> parseDclInputPrimitive(uint32_t opcodeToken,
    ...
    auto inputPrimitive = dxbc::symbolizeInputPrimitive(rawInputPrimitive);
    if (!inputPrimitive)
  ...
  FailureOr<uint32_t> parseGsStreamIndex(SrcOrDstOperand operand,
                                         Location loc) {
    if (operand.getType() != dxbc::OperandType::m)
      return emitError(loc, "unexpected operand type: ")
             << dxbc::stringifyOperandType(operand.getType());
    if (operand.getComponents().getValue() != dxbc::OperandComponents::none)
      return emitError(loc, "unexpected operand components: ")
             << dxbc::stringifyOperandComponents(
  ...
  FailureOr<Instruction> parseDclStream(Location loc) {
    ...
    return builder.buildGsStreamIndexOp<dxbc::DclStream>(*index, loc);
  }
  ...
  struct OperandFields {
    dxbc::OperandType type;
    OperandComponents components;
    SmallVector<dxbc::IndexAttr, 3> indexEntries;
    std::optional<OperandModifier> modifier;
    SmallVector<int32_t, 4> values;
    SmallVector<int64_t, 2> values64;
  };

  FailureOr<dxbc::IndexAttr> parseOperandIndex(uint32_t indexType) {
    switch (indexType) {
    case D3D10_SB_OPERAND_INDEX_IMMEDIATE32: {
  ...
  FailureOr<OperandFields> parseOperandFields() {
    ...
    auto type = dxbc::symbolizeOperandType(rawOperandType);
    if (!type)
  ...
  FailureOr<dxbc::DstOperandAttr> parseDstOperand() {
    auto loc = getLocation();
  ...
  getRequiredImmIndices(SrcOrDstOperand operand, Location loc) {
    ...
    if (auto index = operand.getIndex())
      for (dxbc::IndexAttr entry : index) {
  ...
  FailureOr<Instruction> decodeOp(size_t beginOffset, uint32_t length,
                                  const InstructionModifier &modifier,
                                  Location loc) {
    auto dsts = parseNOperands<NumDstOperands, dxbc::DstOperandAttr>(
        [this] { return parseDstOperand(); });
    FAILURE_IF_FAILED(dsts);
    auto srcs = parseNOperands<NumSrcOperands, dxbc::SrcOperandAttr>(
  ...
  FailureOr<Instruction> parseDclConstantBuffer(uint32_t opcodeToken,
    ...
    auto accessPattern =
        dxbc::symbolizeConstantBufferAccessPattern(rawAccessPattern);
    if (!accessPattern)
  ...
  FailureOr<Instruction> parseDclSampler(uint32_t opcodeToken, Location loc) {
    ...
    auto mode = dxbc::symbolizeSamplerMode(rawMode);
    if (!mode)
      return emitError(loc, "unknown sampler mode: ") << rawMode;

    auto operand = parseDstOperand();
    FAILURE_IF_FAILED(operand);
    if (operand->getType() != dxbc::OperandType::s)
  ...
  FailureOr<dxbc::ResourceReturnType>
  parseResourceReturnType(uint32_t returnTypeToken, uint32_t component,
    ...
    auto returnType = dxbc::symbolizeResourceReturnType(rawReturnType);
    if (!returnType)
  ...
  FailureOr<Instruction> parseDclResource(uint32_t opcodeToken, Location loc) {
    ...
    if (*dim == dxbc::ResourceDimension::texture2dms ||
        *dim == dxbc::ResourceDimension::texture2dmsarray) {
      ...
      if (rawSampleCount == 0)
        return emitError(loc, "sample count must be non-zero for multisampled "
                              "dimension ")
               << dxbc::stringifyResourceDimension(*dim);
      sampleCount = rawSampleCount;
    }

    auto operand = parseDstOperand();
    FAILURE_IF_FAILED(operand);
    if (operand->getType() != dxbc::OperandType::t)
  ...
  FailureOr<UavOperand> parseUavOperand(Location loc) {
    auto operand = parseDstOperand();
    FAILURE_IF_FAILED(operand);
    if (operand->getType() != dxbc::OperandType::u)
...
#define SATURABLE_OP(MNEMONIC, NUM_DST_OPERANDS, NUM_SRC_OPERANDS,             \
                     HAS_PRECISE)                                              \
  decodeOp<dxbc::MNEMONIC, dxbc::MNEMONIC##Sat, HAS_PRECISE, NUM_DST_OPERANDS, \
...
#define PLAIN_OP(MNEMONIC, NUM_DST_OPERANDS, NUM_SRC_OPERANDS, HAS_PRECISE)    \
  decodeOp<dxbc::MNEMONIC, dxbc::MNEMONIC, HAS_PRECISE, NUM_DST_OPERANDS,      \
           NUM_SRC_OPERANDS>(beginOffset, instructionLengthInTokens, modifier, \
                             getLocation())
#define STREAM_INDEX_OP(OP)                                                    \
  decodeStreamIndexOp<dxbc::OP>(beginOffset, instructionLengthInTokens,        \
                                getLocation())
...
#undef STREAM_INDEX_OP
  ...
  FailureOr<Module> parseModule() {
    ...
    dxbc::ProgramTypeAttr programType;
    std::optional<uint32_t> majorVersion;
    std::optional<uint32_t> minorVersion;
    if (*header) {
      programType =
          dxbc::ProgramTypeAttr::get(name.getContext(), (*header)->type);
  ...
  FailureOr<std::optional<ProgramHeader>> parseProgramHeader() {
    ...
    auto programType = dxbc::symbolizeProgramType(rawProgramType);
    if (!programType)
      return std::optional<ProgramHeader>{};
