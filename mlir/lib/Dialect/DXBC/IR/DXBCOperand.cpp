//===--------- DXBCOperand.cpp - DXBC operand attributes ------------------===//
//
...
void DXBCDialect::registerAttributes() {
  addAttributes<
...
Attribute DXBCDialect::parseAttribute(DialectAsmParser &parser,
                                      Type type) const {
...
void DXBCDialect::printAttribute(Attribute attr,
  ...
  if (failed(generatedAttributePrinter(attr, printer)))
    llvm_unreachable("DXBC dialect has no printer for this attribute");
}
